...
export default function Header() {
  ...
  const { setIsConfigModalOpen } = useAPIConfig();
  const { isHistoryOpen, setIsHistoryOpen } = useHistoryPanel();

  return (
    <>
      <motion.header 
        className="fixed top-0 left-0 right-0 h-16 border-b border-[var(--border-color)] bg-[var(--background)]/[.03] backdrop-blur-xl z-30"
        animate={{
          right: isHistoryOpen ? '24rem' : '0',
        }}
      ...
      >
        <div className="max-w-7xl h-full mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-lg">KTranslate</span>
          </div>

          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsHistoryOpen(true)}
              className="p-2 hover:bg-[var(--hover-bg)] rounded-xl transition-colors flex items-center gap-2"
            >
